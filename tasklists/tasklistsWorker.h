...
private:
   ...
   * @return returnCode
   */
  returnCode checkPermission(const std::string& srcUser, const std::string& dstUser, const std::string& tasklist, bool& permission);
...
public:
   ...
   * @param [out] out_list vector of shareInfo with information about the users, tasklists and permissions that current user have access to
   * @return returnCode 
   */
  ...
  /**
   * @brief Create if not exists or Revise the share status for a tasklist. It is guaranteed that 
   * all operations on users before errUser in the in_list are successful
   * 
   ...
   * @return returnCode 
   */
  virtual returnCode GetAllPublicTaskList(std::vector<std::pair<std::string, std::string>>& out_list);
